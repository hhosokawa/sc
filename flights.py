...
def makeGraph(rawdata):
    G, simpleG = tree(), tree()
    for (flightid, origin, dest, depart, arrive, cost) in rawdata:
        G[origin][dest][flightid] = (cost, depart, arrive)
        if origin not in simpleG: simpleG[origin] = set()
        else: simpleG[origin] = simpleG[origin].union([dest])
    return G, simpleG
...
print find_best_flights(all_flights, 'Meekatharra', 'Wiluna')
